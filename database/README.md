...
| `<list key 1>` | list[float] | list y. **이 리스트의 평균이 최소화 대상 목적함수** |
| `<list key 2>` | list[float] | list y. 위와 동일 |
| `<cfg key 1>` | float | batch 공통 설정값. 목적함수가 아니다 |
| `<cfg key 2>` | float | batch 공통 설정값. 목적함수가 아니다 |
...
| npz 키 | dtype | shape |
| --- | --- | --- |
| `<array key 1>` | `bool` | `(batch, i0, i1, i2, i3, i4)` |
| `<array key 2>` | `bool` | `(batch, i0, i1, i2, i3, i4)` |
